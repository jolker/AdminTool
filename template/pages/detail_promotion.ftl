<div class="modal-header">
  ...
  <h4 class="modal-title text-center">Update Promotion</h4>
</div>
      ...
      <div class="row">
        <div class="col-xs-12" style="margin: 0 5%;">
          <div class="form-group">
            ...
            <div class="col-sm-3">
              <input type="text" class="form-control" id="promo_name" placeholder="Title" name="promo_name" value="<#if promotion.promotionName??>${promotion.promotionName}</#if>">
            </div>
            <input type="hidden" id="promo_id" value="<#if promotion.promotionId??>${promotion.promotionId}</#if>">
          </div>
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
            ...
            <div class="col-sm-3">
              <input type="number" class="form-control" id="promo_priority" placeholder="Priority" name="promo_priority" value="<#if promotion.priority??>${promotion.priority}</#if>">
            </div>
          </div>
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
            <label class="col-sm-3 control-label text-right">Status:</label>
            <input type="hidden" value="<#if promotion.status??>${promotion.status}</#if>" class="value-status">
            <div class="col-sm-3">
          ...
          <input type="hidden" value="<#if promotion.dateBegin??>${promotion.dateBegin}</#if>" class="value-modal-date-start">
          <input type="hidden" value="<#if promotion.dateEnd??>${promotion.dateEnd}</#if>" class="value-modal-date-end">
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
          ...
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
            ...
            <input type="hidden" value="<#if promotion.promotionUserGroup??>${promotion.promotionUserGroup}</#if>" class="value-user-group">
            <div class="col-sm-3">
          ...
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
            ...
            <input type="hidden" value="<#if promotion.description??>${promotion.description}</#if>" class="value-description">
            <div class="col-sm-7">
              <textarea class="form-control" rows="5" id="promo_description" placeholder="Description" name="promo_description"></textarea>
            </div>
          </div>
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
            <input type="hidden" value='<#if promotion.promotionRateData??>${promotion.promotionRateData}</#if>' class="value-rate-data">
          ...
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
            ...
            <div class="col-sm-3">
              <p>
                <#if promotion.userCreated??>${promotion.userCreated}</#if>
              </p>
              <input type="hidden" class="form-control" id="promo_user_created" name="promo_user_created" value="<#if promotion.userCreated??>${promotion.userCreated}</#if>" disabled>
            </div>
          </div>
          <div class="col-xs-12 padding-0" style="margin-top: 15px;">
            <label for="description" class="col-sm-3 control-label text-right">UserUpdated:</label>
            <div class="col-sm-3">
              <p>
                <#if sessionInfo??>${sessionInfo.userName}</#if>
              </p>
              <input type="hidden" class="form-control" id="promo_user_modified" name="promo_user_modified" value="<#if sessionInfo??>${sessionInfo.userName}</#if>" disabled>
            </div>
          </div>
        </div>
      </div>
...
<div class="modal-footer">
  <a class="btn btn-primary update-promotion">Update</a>
</div>
...
<script>
  ...
  if ($('.value-user-group').val() != "") {
    $('#promo_user_group').val($('.value-user-group').val()).trigger('change');
  }

  if ($('.value-status').val() != "") {
    $('#promo_status').val($('.value-status').val()).trigger('change');
  }

  if ($('.value-description').val() != "") {
    $('#promo_description').val($('.value-description').val());
  }

  if ($('.value-modal-date-start').val() != "") {
    var date = new Date(parseInt($('.value-modal-date-start').val()));
    $('#promo_date_start').val(dateFormat(date, "yyyy-mm-dd HH:MM:ss"));
  }

  if ($('.value-modal-date-end').val() != "") {
    var date = new Date(parseInt($('.value-modal-date-end').val()));
    $('#promo_date_end').val(dateFormat(date, "yyyy-mm-dd HH:MM:ss"));
  }

  var obj = JSON.parse($('.value-rate-data').val());
  $.each(obj, function(key, value) {
    $("tr[data-id='" + value.ProductKey + "']").find('input.promo-rate').val(value.PromotionRate);
    $("tr[data-id='" + value.ProductKey + "']").find('#promotion_type').val(value.EPromotionType).trigger('change');
  });
  ...
  $(".update-promotion").click(function() {
    ...
    var datas = {
      ...
      "user_modified": $('#promo_user_modified').val()
    }
